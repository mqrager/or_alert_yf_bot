...
_DEFAULT_TICKERS = "SPY,QQQ,TSLA,HOOD,PLTR,MSTR,COIN,ETHE,AMD,NVDA,MSTY,PLTY"
TICKERS: List[str] = [t.strip().upper() for t in os.getenv("TICKERS", _DEFAULT_TICKERS).split(",") if t.strip()]

TIMEZONE_OFFSET = int(os.getenv("TIMEZONE_OFFSET", "-7"))  # PST
...
MAX_DISCORD_LEN = int(os.getenv("MAX_DISCORD_LEN", "1900"))
PRE_MARKET_HOUR = int(os.getenv("PRE_MARKET_HOUR", "6"))
POST_MARKET_HOUR = int(os.getenv("POST_MARKET_HOUR", "13"))
POST_MARKET_MINUTE = int(os.getenv("POST_MARKET_MINUTE", "30"))
POST_MARKET_TIME = (POST_MARKET_HOUR, POST_MARKET_MINUTE)

# HOURLY_RANGE can be "7,8,9,10,11,12,13" or left unset to use range(7,14)
_hr = os.getenv("HOURLY_RANGE", "").strip()
if _hr:
    try:
        HOURLY_RANGE = tuple(int(x) for x in _hr.split(",") if x.strip())
    except Exception:
